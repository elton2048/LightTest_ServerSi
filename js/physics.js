...
var rightPoint;
var wallClosed = true;
var wallSpace = 30; // The width when the wall is broken.
var mirrorSize; // Return the mirror size which is hit by the light
var ray_width = 20 / 2; // Ray width, change the first number only.

// Variable used in the game
var wallWidth = 10;
var closeTime = 150; // When will the wall recover
var startWall_x = 100;
var startWall_y = 100;
var wallLength = 810;
var wallHeight = 600;
...
function update() {
	...
	if(isMouseDown){
		// Denote the wall has been broken
		wallClosed = false;
			...
			if(!fixture.RayCast(output, input) && !rayDone) {
				...
				if(fraction > output.fraction){ 
					...
					switch(body.GetUserData().Type) {
						...
						case "Mirror":
							if(body.GetUserData().Size.x < body.GetUserData().Size.y)
								mirrorSize = body.GetUserData().Size.x;
							else
								mirrorSize = body.GetUserData().Size.y;
							rayReflect = true;
							targetBody = body;
...
function mirrorBuilding(startPoint, size, angle) {
	...
	mirror.userData = {Type: "Mirror", Size: size};
